...
export interface IStorage {
  ...
  getUserFormResponse(formId: number, userId: number): Promise<Response | undefined>;
...
export class DatabaseStorage implements IStorage {
  ...
  async getUserSpaces(userId: number): Promise<(Space & { memberCount: number; role: string })[]> {
    const result = await db
      .select({
        ...
        memberCount: db.$count(spaceMembers, eq(spaceMembers.spaceId, spaces.id)),
      })
      .from(spaces)
  ...
  async getSpaceMemberRole(spaceId: number, userId: number): Promise<string | undefined> {
    const [member] = await db
  ...
  async getUserFormResponse(formId: number, userId: number): Promise<Response | undefined> {
  ...
  async getFormResponseStats(formId: number): Promise<{
    ...
    const memberCount = await db.$count(spaceMembers, eq(spaceMembers.spaceId, form.spaceId));
    const responseCount = await db.$count(responses, and(eq(responses.formId, formId), eq(responses.isDraft, false)));
